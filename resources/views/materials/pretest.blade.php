<x-app-layout>
    <x-slot name="title">Pretest — {{ $material->title }}</x-slot>
    ...
    <div class="min-h-screen py-8" style="background: linear-gradient(135deg, #1e3a5f 0%, #0f4c3a 50%, #2d1b4e 100%);">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="text-center mb-8">
                <div class="inline-flex items-center gap-2 bg-white/10 backdrop-blur text-white text-sm font-bold px-4 py-2 rounded-full mb-4">
                    📋 PRETEST · {{ $material->title }}
                </div>
                <h1 class="text-3xl sm:text-4xl font-extrabold text-white mb-3">Uji Pengetahuan Awalmu!</h1>
                <p class="text-white/70 text-base max-w-lg mx-auto">
                    Sebelum membaca materi, jawab {{ $questions->count() }} soal berikut.
                    Pretest ini <strong class="text-yellow-400">tidak mempengaruhi nilai</strong> — jawab sejujurnya ya! 😊
                </p>
            </div>

<!-- Quiz Form -->
            <form method="POST" action="{{ route('materials.pretest.submit', $material->slug) }}"
                  id="pretest-form"
                  x-data='{
                      current: 0,
                      ...
                      allAnswered() { return Object.keys(this.answers).length === this.total; },
                ...
                @foreach($questions as $i => $question)
                    <div x-show="current === {{ $i }}"
                        ...
                        <div class="bg-gradient-to-r from-amber-500 to-orange-600 px-6 py-4 flex items-center gap-3">
                            <span class="w-9 h-9 bg-white text-orange-600 rounded-xl flex items-center justify-center font-extrabold text-lg shrink-0">
                                {{ $i + 1 }}
                            </span>
                            ...
                            <div class="space-y-3">
                                ...
                                @foreach($shuffledOptions as $idx => $option)
                                    ...
                                    <label
                                        ...
                                        class="flex items-center gap-4 p-4 rounded-xl border-2 hover:border-orange-400 hover:bg-orange-50 dark:hover:bg-orange-900/20 cursor-pointer transition-all group">
                                        <input type="radio"
                                        ...
                                        <span class="w-8 h-8 rounded-lg border-2 flex items-center justify-center font-bold text-sm shrink-0 transition-all"
                                              :class="{
                                                  'border-green-500 bg-green-500 text-white': feedback['{{ $question->id }}'] && '{{ addslashes($option) }}' === feedback['{{ $question->id }}'].correct,
                            ...
                            <div class="flex justify-between mt-8 pt-4 border-t border-gray-100 dark:border-gray-700">
                                ...
                                @if($i < $questions->count() - 1)
                                    <button type="button" @click="current++"
                                            class="flex items-center gap-2 bg-orange-500 hover:bg-orange-400 text-white font-bold text-sm px-5 py-2.5 rounded-xl transition">
                                        Berikutnya →
                                    </button>
                                @else
                                    <button type="submit"
            ...
            <!-- TOMBOL VIP KHUSUS GURU/ADMIN -->
                ...
                <div class="mt-6 bg-yellow-500/20 border border-yellow-400 p-5 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="text-yellow-100 text-sm">
                        ...
                        Kamu bebas melihat-lihat soal menggunakan navigasi angka di atas. Klik tombol di samping untuk langsung melompat ke Materi.
                    </div>
