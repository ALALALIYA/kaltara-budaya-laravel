<x-app-layout>
    <x-slot name="title">Tambah Materi</x-slot>

    <div class="py-8">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">

            <nav class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-6">
                <a href="{{ route('teacher.materials.index') }}" class="hover:text-orange-500 transition-colors">Materi Saya</a>
                <span>/</span>
                <span class="text-gray-700 dark:text-gray-300 font-medium">Tambah Baru</span>
            </nav>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
                ...
                <div class="px-8 py-6 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-orange-50 to-amber-50 dark:from-gray-800 dark:to-gray-800 rounded-t-2xl">
                    <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white">➕ Tambah Materi Baru</h1>
                    <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm">Materi akan masuk antrian review sebelum dipublikasikan ke siswa</p>
                </div>
                ...
                <form method="POST" action="{{ route('teacher.materials.store') }}"
                    ...
                    <div class="p-8 space-y-8">
                        ...
                        <div>
                            ...
                            <input id="title" name="title" type="text"
                                   value="{{ old('title') }}"
                                   placeholder="Contoh: Tari Hudoq – Ritual Sakral Suku Dayak Kalimantan Utara"
                                   required
                                   class="block w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-orange-500 focus:ring-orange-500 text-base py-3 px-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <select id="category" name="category"
                                    ...
                                    <option value="dayak"  {{ old('category') === 'dayak'  ? 'selected' : '' }}>🦅 Dayak</option>
                                    <option value="banjar" {{ old('category') === 'banjar' ? 'selected' : '' }}>🎋 Banjar</option>
                                    <option value="kutai"  {{ old('category') === 'kutai'  ? 'selected' : '' }}>🐉 Kutai</option>
                                    <option value="tidung" {{ old('category') === 'tidung' ? 'selected' : '' }}>🌊 Tidung</option>
                                </select>
                            ...
                            <div>
                                ...
                                <textarea id="description" name="description" rows="3" required
                                          placeholder="Ringkasan singkat tentang materi ini (tampil di halaman daftar materi)"
                                          class="block w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-orange-500 focus:ring-orange-500">{{ old('description') }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea id="kompetensi_dasar" name="kompetensi_dasar" rows="2"
                                      ...
                                      class="block w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-orange-500 focus:ring-orange-500">{{ old('kompetensi_dasar') }}</textarea>
                        ...
                        <!-- Konten Materi — Full Width, Spacious Editor -->
                        <div>
                            ...
                            <p class="text-xs text-gray-400 dark:text-gray-500 mb-3">
                                Tulis materi lengkap di sini. Kamu bisa menyisipkan gambar (drag & drop), video YouTube, tabel, list, dan format teks lainnya.
                            </p>
                            <div class="w-full">
                                <textarea id="content" name="content" required
                                          class="block w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                          placeholder="Tulis konten materi di sini...">{{ old('content') }}</textarea>
                            </div>
                            <x-input-error :messages="$errors->get('content')" class="mt-1" />
                        </div>

                        <!-- Row: Gambar + Audio + Video URL -->
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <!-- Gambar -->
                            <div>
                                <label for="image" class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                                    Gambar Sampul <span class="text-gray-400 text-xs font-normal">(opsional)</span>
                                </label>
                                <p class="text-xs text-gray-400 mb-3">JPEG/PNG/WebP, maks. 2MB</p>

                                <div x-show="imagePreview" class="mb-3 w-full h-40 rounded-xl overflow-hidden border-2 border-orange-300">
                                    <img :src="imagePreview" alt="Preview" class="w-full h-full object-cover">
                                </div>
                                <div x-show="!imagePreview"
                                     class="mb-3 w-full h-40 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-600 flex items-center justify-center text-gray-300 dark:text-gray-600">
                            ...
                            <div x-data="{ hasAudio: false }">
                                ...
                                <div class="mb-3 w-full h-40 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-600 flex flex-col items-center justify-center text-gray-300 dark:text-gray-600 transition-colors"
                                     :class="hasAudio ? 'border-green-400 bg-green-50/50 dark:bg-green-950/20' : ''">
                                    <span class="text-4xl" :class="hasAudio ? 'animate-bounce text-green-600 dark:text-green-400' : ''">🎵</span>
                                    <span class="text-xs font-semibold mt-2" :class="hasAudio ? 'text-green-600 dark:text-green-400' : 'text-gray-400'">
                                        <span x-show="!hasAudio">Belum Ada Audio</span>
                                        <span x-show="hasAudio">Audio Terpilih!</span>
                                    </span>
                                </div>
                            ...
                            <!-- Video URL -->
                            <div>
                                <label for="video_url" class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                                    URL Video YouTube <span class="text-gray-400 text-xs font-normal">(opsional)</span>
                                </label>
                                <p class="text-xs text-gray-400 mb-3">
                                    Gunakan link Embed YouTube:<br>
                                    <code class="text-xs bg-gray-100 dark:bg-gray-700 px-1 rounded">https://www.youtube.com/embed/VIDEO_ID</code>
                                </p>
                    ...
                    <div class="px-8 py-5 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 rounded-b-2xl flex gap-3">
                        <button type="submit"
                                class="inline-flex items-center gap-2 px-6 py-3 bg-orange-600 hover:bg-orange-500 text-white font-bold rounded-xl shadow-md hover:shadow-orange-500/30 transition text-sm">
    ...
    <style>
        ...
        .note-btn { background-color: white !important; border: 1px solid #d1d5db !important; color: #374151 !important; border-radius: 0.375rem !important; font-size: 13px !important; padding: 4px 8px !important; transition: all 0.15s; }
        .dark .note-btn { background-color: #4b5563 !important; border-color: #4b5563 !important; color: #f3f4f6 !important; }
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#content').summernote({
                placeholder: 'Tulis materi di sini... Kamu bisa drag & drop gambar langsung ke editor ini.',
                tabsize: 2,
                height: 550,
                toolbar: [
                    ['style',  ['style']],
            ...
            function uploadImage(file) {
                ...
                $.ajax({
                    ...
                    error: function (xhr) {
                        alert('Gagal upload gambar: ' + (xhr.responseJSON?.message || 'Cek ukuran / format file.'));
                    }
                });
            }
        });
